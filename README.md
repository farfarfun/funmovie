...
- `funmovie/magnet/crawler.py` 实现了一个简化版 DHT 爬虫（`DHTServer`），用于直接从 BT 网络里嗅探 info_hash；该文件顶层直接调用了 `start_server()`，`magnet_to_torrent_aria2c.py` 顶层也直接调用了 `magnet2torrent()` —— 这两个文件按脚本方式运行即可，但 `import` 它们会立刻触发网络/RPC 调用，不是纯粹的库代码，使用前请注意。
